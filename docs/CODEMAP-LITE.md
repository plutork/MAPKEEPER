...
- **River flow direction chevrons (D-103)** — chevron on every path edge + `mouth_extensions` land→ocean stroke from elevation neighbors; presentation only, amends D-102 -> `crates/core/src/worldgen/hydrology/render.rs`, `crates/server/src/rivers.rs`, `crates/web/src/canvas.rs` (`hydrology-river-flow-arrows`)
