...
- Step-4 geology model (`geology` categorical, **D-87 hidden plate substrate** in `plates.rs`, styles belts/shields/arcs/random, boundary-driven orogenic chains + coast arcs + anti-speckle, elevation bridge) -> `crates/core/src/geology.rs`, `crates/core/src/plates.rs` (`hidden-plates-geology-foundation`, D-63, D-72, D-87)
