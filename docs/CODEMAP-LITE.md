...
- Step-4 geology model (`geology` categorical, **D-87 hidden plate substrate** in `plates.rs`, boundary-distance field, probabilistic orogenic width/gaps along plate edges, styles belts/shields/arcs/random, elevation bridge) -> `crates/core/src/geology.rs`, `crates/core/src/plates.rs` (`hidden-plates-geology-foundation`, D-63, D-72, D-87)
