...
- Generated rivers (final `DrainageGraph` → accumulation → `RiverGraph` only; old owner/path-first generator removed) -> `crates/core/src/worldgen/hydrology/drainage_graph.rs`, `channel_graph.rs`, `catalog.rs`, `snapshot.rs`, `crates/server/src/rivers.rs` (`hydrology-v2--legacy-removal`, D-101; legacy catalog read only for migration)
...
- **Hydrology v2 catalog/renderer cutover** — generated physical segment bindings, compatible `river_id` projection, name-overlap migration report, and v2-only river generation/render data -> `crates/core/src/worldgen/hydrology/catalog.rs`, `crates/server/src/rivers.rs`, `crates/web/src/api.rs`, `crates/web/src/canvas.rs` (`hydrology-v2--catalog-renderer-cutover`, D-101; legacy catalog read only for migration)
...
- River generate API (`POST /api/rivers/generate` — replace-all; optional `{ river_density, regenerate_nonce }`; persists one v2 snapshot plus compatibility `river_id`; `rivers.json` is migration input only) -> `crates/server/src/rivers.rs`, `world_io.rs` (D-101)
