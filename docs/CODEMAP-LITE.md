...
- **Hydrology v2 diagnostics baseline** — read-only legacy metrics: depression/lake/river component counts, terminal reasons, short coastal-river heuristic, and catalog↔dense-layer mismatch -> `crates/core/src/worldgen/hydrology/diagnostics.rs`, `crates/server/src/hydrology_diagnostics.rs` (`GET /api/hydrology/diagnostics`; `hydrology-v2--diagnostics-baseline`, D-101)
