...
- Elevation-driven river auto-generation (flux, confluence, `parent`/`basin`; **D-91** reads `precipitation` when present, uniform fallback; **lake-aware routing** + `RiverDensity` Few/Balanced/Many) -> `crates/core/src/worldgen/hydrology/river_flux.rs`, `types.rs` (`rivers-auto-from-elevation-v1`, D-55; `rivers-flux-v2--climate-precip`, D-91; `hydrology-river-lake-integration-v1`; legacy `mapkeeper_core::river_flux`)
...
- River generate API (`POST /api/rivers/generate` — replace-all; optional `{ river_density, regenerate_nonce }`; reads lake catalog when present; `precip_source` + `river_density` in response) -> `crates/server/src/rivers.rs` (`rivers-auto-from-elevation-v1`, D-55; D-91 climate precip; `hydrology-river-lake-integration-v1`)
